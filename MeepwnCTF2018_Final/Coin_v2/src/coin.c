...
{
	pCurencies = 0;
	AddCurrency("BTC",6522.97,0.0002);
	AddCurrency("BCH",528.32,0.0002);
	AddCurrency("ETH",274.71,0.0002);
	AddCurrency("LTC",56.87,0.0002);
	AddCurrency("EOS",4.89,0.0002);
...
void OpenOrder()
{
	...
	else if(type == 1)
	{
		...
		scanf("%lf",&tmpPrice);
		nOrder->is_pending = 1;
		if(tmpPrice < pCurency->price*0.1 )
		{
			printf("Sryyyy... I dont think the price will be decreased  10 times :)\n");
			goto FreeOOder;
		}
		if(tmpPrice > pCurency->price*10 )
		{
			printf("Sryyyy... I dont think the price will be increased 10 times :)\n");
			goto FreeOOder;	
		}
	}
	else goto FreeOOder;
...
void *Change_Price(void *ptr)
{
	...
	while(true)
	{
		...
		while(pCur)
		{
			...
			// if (!strcmp("SUN",pCur->name))
			// 	printf("%s: %d %0.4f*%0.4f=%0.4f \n",pCur->name,x, percent,pCur->price,percent*pCur->price);
